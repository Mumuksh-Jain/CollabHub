...
export default function MyProjects() {
  ...
  const handleRespond = async (projectId, userId, action) => {
    try {
      ...
      fetchMyProjects();
      setActiveTab(action === 'accept' ? 'joined' : 'pending');
    } catch (err) {
  ...
  const handleInviteResponse = async (projectId, action) => {
    try {
      await projectAPI.respondInvite(projectId, action);
      // Remove from invites list immediately
      setInvites(prev => prev.filter(p => p._id !== projectId));
      if (action === 'accept') {
        fetchMyProjects(); // refresh joined projects
        setActiveTab('joined');
      }
    } catch (err) {
  ...
  const tabs = [
    ...
    { key: 'invites',  label: '📬 Invites', count: invites.length              || 0 },
      ...
      <div className="tabs">
        {tabs.map(tab => (
          ...
          >
            {tab.label}
            {tab.count > 0 && (
              <span className={`tab-badge ${tab.key === 'invites' ? 'tab-badge-highlight' : ''}`}>
                {tab.count}
              </span>
            )}
          </button>
            ...
            <div key={project._id} className="project-card project-card-detailed">
              <div className="project-card-top">
                <div>
                  <Link
                    ...
                    state={{ project }}
                    style={{ textDecoration: 'none', color: 'inherit', opacity: 1 }}
                  >
                    <h3 className="project-title">{project.title}</h3>
                  </Link>
                  <div className="project-info-sections" style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div className="info-group">
                      <h4 style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: '8px' }}>
                        Skills Required
                      </h4>
                      <div className="project-tags">
                        {project.tech_stack?.map((t, i) => <span key={i} className="tag tag-tech">{t}</span>)}
                      </div>
                    </div>
                    <div className="info-group">
                      <h4 style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: '8px' }}>
                      ...
                      <div className="project-tags">
                        {project.roles_needed?.map((r, i) => <span key={i} className="tag tag-role">{r}</span>)}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="project-section">
                ...
                {project.members?.length > 0 ? (
                  ...
                  <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '8px' }}>No members yet</p>
                )}
              </div>
          ...
          {data.joinedProjects?.map(project => (
            <div key={project._id} className="project-card">
              <div className="project-card-header">
                <Link to={`/project/${project._id}`} state={{ project }} style={{ textDecoration: 'none', color: 'inherit', opacity: 1 }}>
                  <h3 className="project-title">{project.title}</h3>
                </Link>
        ...
        <div className="projects-list stagger-children">
          {invites.length === 0 && (
            <div className="empty-state fade-in">
              ...
              <p>When a project creator invites you, it will appear here.</p>
            </div>
          )}
          {invites.map(project => (
            <div
              ...
              style={{ border: '1px solid var(--accent)', position: 'relative' }}
            >
              {/* "Invited" badge */}
              <span style={{
              ...
              <div className="project-card-header">
                <Link to={`/project/${project._id}`} style={{ textDecoration: 'none', color: 'inherit', opacity: 1 }}>
                  <h3 className="project-title">{project.title}</h3>
                </Link>
                <span className="project-author">by {project.created_by?.name || 'Unknown'}</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '14px' }}>
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => handleInviteResponse(project._id, 'accept')}
                >
                  ✓ Accept
                </button>
                <button
                  className="btn btn-outline btn-sm"
                  onClick={() => handleInviteResponse(project._id, 'decline')}
                >
                  ✗ Decline
                </button>
              </div>
          ...
          {data.removedFromProjects?.map(project => (
            <div key={project._id} className="project-card" style={{ border: '2px solid var(--accent-red)', position: 'relative' }}>
              ...
              <div style={{
                ...
                color: 'var(--accent-red)', fontWeight: '600', fontSize: '0.9rem'
              }}>
